...
const Transactions = () => {
  ...
  return (
    <div className='flex w-full justify-center items-center 2xl:px-20 gradient-bg-transactions'>
      <div className='flex flex-col md:p-12 ppy-12 px-4'>
        {currentAccount ? (
          <h3 className='text-white text-3xl text-center py-2'>
            Latest transactions
          </h3>
        ) : (
          <h3 className='text-white text-3xl text-center py-2'>Connect your account to see latest transactions</h3>
        )}
        <div className='flex flex-wrap justify-center items-center mt-10'>
          {transactions.reverse().map((transaction, ind) => (
            <TransactionCard key={ind} {...transaction} />
          ))}
        </div>
      </div>
    </div>
